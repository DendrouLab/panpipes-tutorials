...
`panpipes ingest config`
This command will generate a config file, `pipeline.yml`. Modify the config file, or just use sample submission file we provided. You can find the preconfigured `pipeline.yml` file [here](../ingesting_multiomodal_data/pipeline_yml.md). 
